...
var hunger_depletion_per_tile: float = 1.5
...
func tick_step(active_party_ids: Array) -> void:
	for id in active_party_ids:
		...
		if s.current_hunger > 0.0:
			s.current_hunger = max(0.0, s.current_hunger - hunger_depletion_per_tile)
...
func restore_hunger(id: StringName, amount: float) -> void:
	var s: StudentData = PartyManager.get_student(id)
	if s == null:
		return
